...
class ShipmentRequestMapper
{
    ...
    public function map(ShipmentRequestInterface $request)
    {
        ...
        $shipmentInfo = new ShipmentInfo(
            ...
            $request->getShipmentDetails()->isPickupDetailsRequested(),
        ...
        // print_r(new ExportLineItems(
        //     $this->mapExportLineItems($request->getExportLineItems())
        // )); die;
        $exportDeclaration = new InternationalDetail\ExportDeclaration(
            new ExportLineItems(
                $this->mapExportLineItems($request->getExportLineItems())
            ),
            $request->getExportDeclaration()->getInvoiceDate(),
            $request->getExportDeclaration()->getInvoiceNumber(),
            $request->getExportDeclaration()->getExportReasonType(),
            $request->getExportDeclaration()->getExportReason(),
        );

        // print_r($exportDeclaration); die;
        ...
        if ($labelOptions instanceof LabelOptionsInterface) {
            $requestedShipment->getShipmentInfo()->setLabelOptions(
                new LabelOptions(
                    ...
                    new LabelOptions\RequestDHLCustomsInvoice($labelOptions->isDHLCustomsInvoiceRequested()),
                    new LabelOptions\DHLCustomsInvoiceType($labelOptions->getDHLCustomsInvoiceType())
                )
            );
        }

        // print_r($requestedShipment); die;
    ...
    /**
     * @param ExportLineItemInterface[] $exportLineItem
     *
     * @return ExportLineItem[]
     */
    // private function mapExportLineItems(array $exportLineItems)
    // {
    //     $exportLineItemsArr = null;
    //     foreach ($exportLineItems as $exportLineItem) {
    //         $exportLineItemsArr = new ExportLineItem(
    //             $exportLineItem->getItemNumber(),
    //             $exportLineItem->getCommodityCode(),
    //             $exportLineItem->getQuantity(),
    //             $exportLineItem->getQuantityUOM(),
    //             $exportLineItem->getItemDescription(),
    //             $exportLineItem->getUnitPrice(),
    //             $exportLineItem->getNetWeight(),
    //             $exportLineItem->getGrossWeight(),
    //             $exportLineItem->getManufacturingCountryCode(),
    //             $exportLineItem->getExportReasonType(),
    //         );
    //     }

    //     // print_r($exportLineItemsArr); die;
    //     return $exportLineItemsArr;
    // }
    private function mapExportLineItems(array $exportLineItems)
    {
        $exportLineItemsArr = [];
