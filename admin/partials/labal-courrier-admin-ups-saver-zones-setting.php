                ...
                <div class="row">
                    <div class="col-12">
                        <div class="accordion rate-setting" id="zone_setting">
                            ...
                            foreach ($zoneObj->get_ups_saver_export_zones() as $zone_id => $zone) :

                                if (isset($user_coeffient_page) && $user_coeffient_page == 1) {
                                    $existingZoneValues = get_user_meta($_GET['user_id'], 'ups_saver_export_rate_' . $zone_id, true);
                                } else {
                                    $existingZoneValues = get_option('ups_saver_export_rate_' . $zone_id);
                                }
                                    ...
                                    <div id="collapse_e<?= $zone_id ?>" class="collapse <?= $show ?>" aria-labelledby="zone<?= $zone_id ?>" data-parent="#zone_setting">
                                        <div class="card-body">
                                            <form action="<?php echo admin_url() . 'admin-post.php'; ?>" method="POST" data-zone-id="zone<?= $zone_id ?>" class="form-rate form-zone-<?= $zone_id ?>">
                                                ...
                                                <input type="hidden" value="saver" name="service_code" />
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="accordion rate-setting" id="zone_i_setting">
                            ...
                            foreach ($zoneObj->get_ups_saver_import_zones() as $zone_id => $zone) :

                                if (isset($user_coeffient_page) && $user_coeffient_page == 1) {
                                    $existingZoneValues = get_user_meta($_GET['user_id'], 'ups_saver_import_rate_' . $zone_id, true);
                                } else {
                                    $existingZoneValues = get_option('ups_saver_import_rate_' . $zone_id);
                                }
