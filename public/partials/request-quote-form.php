...
if (isset($_GET['request_id']) && !empty($_GET['request_id'])) {
    ...
    if ($lc_quote_id != '') {
        ...
        if (isset($data['quote_request']) && !empty($data['quote_request'])) {
            ...
            $messages['old_values'] = (array) $q_data;
        }
    }
}
// echo '<pre>';
...
<div x-data="component()" x-init="init()" class="lc-home-step-area lc-form-container">
    <div class="lc-home-step-wrapper">
        <div class="lc-home-step-grid py-5 gap-2 md-gap-5">
            <div class="lc-home-step-grid-man grid-align-self-end grid-justify-self-center">
                <img src="<?= LABAL_COURRIER_PLUGIN_URL . '/public/img/delivery-man.svg' ?>" alt="<?= esc_attr_e("Delivery Man", "labal-courrier") ?>">
            </div>
            <div class="lc-home-step-grid-content shadow-sm rounded grid-align-self-end">
                <img class="quote-form-img quote-form-img-top" src="<?= LABAL_COURRIER_PLUGIN_URL . '/public/img/cloud.svg' ?>" alt="<?= esc_attr_e("Cloud", "labal-courrier") ?>">
                <div class="quote-form-heading lc-grid grid-cols-2 text-center">
                    <!-- <div x-on:click="formData.quote_type = 'full'; document.getElementById('id_calendar_trigger').value = ''; document.getElementById('id_gq_dispatch_date').value = ''; showCalendar = false" :class="{ 'active': formData.quote_type == 'full' }" class="heading-full-quote p-2"> -->
                    <div x-on:click="onChangeQuoteType('full')" :class="{ 'active': formData.quote_type == 'full' }" class="heading-full-quote p-2">
                        <i class="fa-solid fa-arrow-down-wide-short"></i>
                        <span><?= esc_html_e("Full Quote", "labal-courrier") ?></span>
                    </div>
                    <div x-on:click="onChangeQuoteType('quick')" :class="{ 'active': formData.quote_type == 'quick' }" class="heading-quick-quote p-2">
                        <i class="fa-solid fa-plane"></i>
                        <span><?= esc_html_e("Quick Quote", "labal-courrier") ?></span>
                    </div>
                </div>
                <div class="quote-form-area">
                    <form id="frm_get_quote" class="lc-form wpcsr-quote-book mb-0" action="<?php echo esc_url(site_url('wp-admin/admin-post.php')); ?>" method="post">
                        ...
                        <input x-model="formData.quote_type" type="hidden" name="quote_type" value="">
                        <?php wp_nonce_field('get_quote', 'lc_nonce'); ?>
                        <?php
                        if (isset($atts['sender']) && $atts['receiver']) {
                        ?>
                            <input type="hidden" name="is_country_page" value="1">
                        <?php
                        }
                        ?>
                        ...
                        <div x-show="formData.quote_type == 'quick'" class="quote-form-quick lc-grid gap-4 p-4 bg-white">
                            <div class="form-quick-left">
                                <div class="form-quick-left-wrapper lc-grid gap-2">
                                    <div class="lc-form-control">
                                        <label for=""><?= esc_html_e("From", "labal-courrier") ?></label>
                                        <select class="form-control lc-select-country col_country_quick" name="col_country_quick" id="col_country_from_quick" aria-label="Example select with button addon">
                                            <option value="" selected><?= esc_html_e("Choix du pays", "labal-courrier") ?></option>
                                            <?php

                                            foreach ($countries as $code => $name) : ?>
                                                <?php
                                                if (isset($messages['old_values']['col_country_quick']) && trim($messages['old_values']['col_country_quick']) == $code) : ?>
                                        ...
                                        <div x-show="!formvalidation.col_country_from_quick.validated" class="w-100 text-danger validation_message"><?= esc_html_e("This field cannot be empty", "labal-courrier") ?></div>
                                    </div>
                                    <div class="lc-form-control">
                                        <label for=""><?= esc_html_e("To", "labal-courrier") ?></label>
                                        <select class="form-control lc-select-country del_country_quick" name="del_country_quick" id="col_country_to_quick" aria-label="Example select with button addon">
                                            <option value="" selected><?= esc_html_e("Choix du pays", "labal-courrier") ?></option>

                                            <?php foreach ($countries as $code => $name) : ?>
                                                <?php if (isset($messages['old_values']['del_country_quick']) && trim($messages['old_values']['del_country_quick']) == $code) : ?>
                                                    <option selected value="<?= $code ?>"><?= $name ?></option>
                                                <?php else : ?>
                                                    <option value="<?= $code ?>"><?= $name ?></option>
                                                <?php endif; ?>
                                        ...
                                        <div x-show="!formvalidation.col_country_to_quick.validated" class="w-100 text-danger validation_message"><?= esc_html_e("This field cannot be empty", "labal-courrier") ?></div>
                                    </div>
                                    <div class="lc-form-control">
                                        <label for="id_quick_weight"><?= esc_html_e("Weight", "labal-courrier") ?></label>
                                        <span class="measurement-instruction"><input x-model="formData.weight" type="text" name="quick_weight" id="id_quick_weight" class="validate_number"><span><?= esc_html_e("kg", "labal-courrier") ?></span></span>
                                        <div x-show="!formvalidation.weight.validated" class="w-100 text-danger validation_message"><?= esc_html_e("This field cannot be empty", "labal-courrier") ?></div>
                                    </div>
                                </div>
                            </div>
                            <div class="form-quick-right text-end">
                                <button x-on:click.prevent="onSubmitQuick()" type="button" class="btn-quote rounded py-2 w-100"><?= esc_html_e("Quote", "labal-courrier") ?></button>
                            </div>
                        </div>
                        <div x-show="formData.quote_type == 'full'" class="quote-form-full bg-white">
                            ...
                            <div class="full-quote-bottom p-4">
                                <div class="full-quote-from-to lc-grid grid-cols-1 md-grid-cols-2 md-gap-5 lg-gap-10">
                                    <div class="full-quote-from">
                                        <div class="lc-form-control bg-grey" :class="{ 'is-invalid-select': !formvalidation.col_country_from.validated }">
                                            <label for=""><?= esc_html_e("From", "labal-courrier") ?></label>
                                            <select x-model="formData.col_country_from" class="form-control lc-select-country col_country_full" name="col_country_full" id="col_country_from" aria-label="Example select with button addon">
                                                <option value="" selected><?= esc_html_e("Choix du pays", "labal-courrier") ?></option>
                                                <?php foreach ($countries as $code => $name) : ?>
                                                    <?php if (isset($messages['old_values']['col_country_full']) && trim($messages['old_values']['col_country_full']) == $code) : ?>
                                                        <option selected value="<?= $code ?>"><?= $name ?></option>
                                                    <?php else : ?>
                                                        <option value="<?= $code ?>"><?= $name ?></option>
                                                    <?php endif; ?>
                                    ...
                                    <div class="full-quote-to mt-2 mt-md-0">
                                        <div class="lc-form-control bg-grey" :class="{ 'is-invalid-select': !formvalidation.col_country_to.validated }">
                                            <label for=""><?= esc_html_e("To", "labal-courrier") ?></label>
                                            <select x-model="formData.col_country_to" class="form-control lc-select-country del_country_full" name="del_country_full" id="col_country_to" aria-label="Example select with button addon">
                                                <option value="" selected><?= esc_html_e("Choix du pays", "labal-courrier") ?></option>

                                                <?php foreach ($countries as $code => $name) : ?>
                                                    <?php if (isset($messages['old_values']['del_country_full']) && trim($messages['old_values']['del_country_full']) == $code) : ?>
                                                        <option selected value="<?= $code ?>"><?= $name ?></option>
                                                    <?php else : ?>
                                                        <option value="<?= $code ?>"><?= $name ?></option>
                                                    <?php endif; ?>
                                ...
                                <div class="full-quote-insurance lc-grid grid-cols-1 md-grid-cols-2 md-gap-5 lg-gap-10 mt-3 ">
                                    <div class="lc-form-control">
                                        <label for=""><?= esc_html_e("Insurance", "labal-courrier") ?><i class="fa-regular fa-circle-question" data-bs-delay='{"show":0,"hide":500}' data-bs-custom-class="lc-tooltip" data-bs-toggle="tooltip" data-bs-html="true" data-bs-placement="top" data-bs-title="<?= esc_attr_e("<strong>Vous avez choisi de ne pas assurer votre envoi!</strong> <br>En cas de perte ou de dommage, vous n'aurez droit qu'à une indemnité contractuelle de la part du transporteur (maximum 25 dollars/kg si l'envoi est livré par avion ou 12 dollars/kg s'il est transporté par route).<br><br> <strong>Vous avez choisi d'assurer votre envoi!</strong><br>Remboursement de 100% de la valeur déclarée<br>Couverture en cas de perte ou de dommage<br>Procédure de remboursement simplifiée", "labal-courrier") ?>"></i></label>
                                ...
                                <div class="full-quote-pickup-type lc-grid grid-cols-1 md-grid-cols-2 md-gap-5 align-items-end lg-gap-10 mt-3 ">
                                    <div class="lc-form-control-group">
                                        ...
                                        <div x-show="formData.is_pickup_required == 1" class="lc-form-control mt-3" id="dispatch_date">
                                            ...
                                            <input type="hidden" id="id_gq_dispatch_date_quick" class="gq-dispatch-date-field-quick" name="dispatch_date_quick" value="" />
                                            <input type="hidden" id="id_gq_dispatch_date_full" class="gq-dispatch-date-field-full" name="dispatch_date_full" value="" />
            ...
            <div class="lc-home-step-grid-box grid-align-self-end grid-justify-self-center">
                <img src="<?= LABAL_COURRIER_PLUGIN_URL . '/public/img/box.svg' ?>" alt="<?= esc_attr_e("Delivery Box", "labal-courrier") ?>">
            </div>
        </div>
    </div>
</div>
    ...
    function component() {
        return {
            ...
            init() {
                ...
                jQuery(document).on('change', 'select[name="col_country_full"]', (e) => {
                    const col_country = jQuery('#col_country_from').val();
                    if (col_country) this.initiateOrUpdateCalendar();
                });

                jQuery(document).on('change', 'select[name="col_country_quick"]', (e) => {
                    const col_country = jQuery('#col_country_from_quick').val();
                    if (col_country) this.initiateOrUpdateCalendar();
                });

                const quote_type = '<?= isset($q_data->quote_type) ? $q_data->quote_type : "quick" ?>';
                ...
                if (quote_type == 'quick') {
                    ...
                    if (isset($atts['sender']) && $atts['receiver']) {
                    ?>
                        col_country = '<?= $atts['sender'] ?>';
                        del_country = '<?= $atts['receiver'] ?>';
                    <?php } else { ?>
                        col_country = '<?= isset($q_data->col_country_quick) ? $q_data->col_country_quick : "" ?>';
                        del_country = '<?= isset($q_data->del_country_quick) ? $q_data->del_country_quick : "" ?>';
                    <?php } ?>
                }
                ...
                setTimeout(() => {
                    ...
                    // if (this.formData.quote_type == 'full') {
                    ...
                    // } 
                    if (this.formData.quote_type == 'quick') {
                        jQuery('#col_country_from_quick').val(col_country).change();
                        jQuery('#col_country_to_quick').val(del_country).change();
                    }
                ...
                setTimeout(() => {
                    ...
                    // if (package != '' && JSON.parse(package).length > 0 && quote_type == 'full') {
            ...
            getMaxDay($format = "DD/MM/YYYY") {
                ...
                <?php
                if (isset($q_data->dispatch_date) && $q_data->dispatch_date != '') {
                ?>
                    if (this.formData.quote_type == 'full') {
                        jQuery('.gq-dispatch-date-field-full').val('<?= $q_data->dispatch_date ?>');
                    } else {
                        jQuery('.gq-dispatch-date-field-quick').val('<?= $q_data->dispatch_date ?>');
                    }

                <?php } else { ?>
                    if (this.formData.quote_type == 'full') {
                        jQuery('.gq-dispatch-date-field-full').val(moment(this.calendarStartDay, 'DD-MM-YYYY').format('YYYY-MM-DD'));
                    } else {
                        jQuery('.gq-dispatch-date-field-quick').val(moment(this.calendarStartDay, 'DD-MM-YYYY').format('YYYY-MM-DD'));
                    }
            ...
            async getHolidays() {
                ...
                let col_country_from;

                if (this.formData.quote_type == 'full') {
                    col_country_from = jQuery('#col_country_from').val();
                } else {
                    col_country_from = jQuery('#col_country_from_quick').val();
                }
            ...
            async initiateOrUpdateCalendar() {
                ...
                <?php if (isset($q_data->dispatch_date) && $q_data->dispatch_date != '') { ?>
                    jQuery('.gq-dispatch-date').daterangepicker({
                        ...
                        if (this.formData.quote_type == 'full') {
                            jQuery('.gq-dispatch-date-field-full').val(moment(start).format('YYYY-MM-DD'));
                        } else {
                            jQuery('.gq-dispatch-date-field-quick').val(moment(start).format('YYYY-MM-DD'));
                        }
                    });
            ...
            errorMessage: [],
            formData: {
                quote_type: '<?= isset($q_data->quote_type) ? $q_data->quote_type : "quick" ?>',
                col_country_from: '',
                col_country_to: '',
                ...
                insurance_value: '',
                is_pickup_required: '',
                weight: '<?= (isset($q_data->quote_type) && $q_data->quote_type == 'quick' && $q_data->quick_weight != '') ? $q_data->quick_weight : "" ?>',
                dispatch_date: '',
                packages: [{
                    weight: '',
                    length: '',
            ...
            onSubmit() {
                ...
                const dispatch_date = jQuery('.gq-dispatch-date-field-full').val();
            ...
            onSubmitQuick() {
                ...
                // get all the values 
                const col_country_from = this.formData.col_country_from = jQuery('#col_country_from_quick').val();
                const col_postcode_or_city = this.formData.col_postcode_or_city = jQuery('#col_postcode_or_city').val();
                const col_country_to = this.formData.col_country_to = jQuery('#col_country_to_quick').val();
                const del_postcode_or_city = this.formData.del_postcode_or_city = jQuery('#del_postcode_or_city').val();
                const package_type = this.formData.package_type = 'Package';
                const is_pickup_required = this.formData.is_pickup_required = 0;
                const insurance = this.formData.insurance = 0;
                const insurance_value = this.formData.insurance_value = 0;

                if (col_country_from == '') {
                    this.formvalidation.valid = false;
                    this.formvalidation.col_country_from_quick.validated = false;
                    this.validationIds.push('col_country_from_quick');
                }
                if (col_country_to == '') {
                    this.formvalidation.valid = false;
                    this.formvalidation.col_country_to_quick.validated = false;
                    this.validationIds.push('col_country_to_quick');
                }

                if (this.formData.weight == '') {
                    this.formvalidation.valid = false;
                    this.formvalidation.weight.validated = false;
                    this.validationIds.push('id_quick_weight');
                }
